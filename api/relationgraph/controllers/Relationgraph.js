  ...
  creates: async (ctx) => {
    ...
    if (bodyData.edges.length > 1) {
      for (let index = 1; index < bodyData.edges.length; index++) {
        ...
        query_betw =
          query_betw +
          `,('${edge.source}','${edge.destination}','${edge.node_name}','${edge.node_request}','${edge.data_key}','${edge.source_label}','${edge.destination_label}','${edge.destination_method}', '${edge.source_method}','${edge.application}')`;
      }
    }
    const query = query_start + query_betw + query_end;
    console.log(query);
